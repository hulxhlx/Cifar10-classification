...
class CNNSVM:
    def __init__(self, alpha, batch_size, num_classes, num_features, penalty_parameter):
        ...
        def __graph__():
            ...
            #     testshape = tf.placeholder(
            #         dtype=tf.int32
            #     )
            #     regularizationtest_loss = tf.reduce_mean(tf.square(readout_weight))
            #     hingetest_loss = tf.reduce_mean(
            #         tf.square(
            #             tf.maximum(
            #                 tf.zeros([testshape, num_classes]), 1 - y_input * output
            #             )
            #         )
            #     )
            #     test_loss = regularizationtest_loss + penalty_parameter * hingetest_loss
            #
        ...
        with tf.Session() as sess:
            ...
            for index in range(epochs):
                ...
                if index % 400 == 0 and index != 0:
                    ...
                    # train_writer.add_summary(summary=train_accuracy, global_step=index/400)
